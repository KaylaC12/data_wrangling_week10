...
c.  The function in (b) assumes that the weight is provided in grams. Use the functions from (3a) and (3b) in combination to estimate the height for an animal that weighs 2 pounds using the default value for `a`, but changing the value for `b` to 0.32.
    ...
    #3c
    weight_grams <- convert_pounds_to_grams(2)
    get_height_from_weight(weight_grams, b = 0.32)
    ```
...
#5b
mass_kg <- get_mass_from_length(12, a = 10.95, b = 2.64) 
  convert_kg_to_pounds(mass_kg)
```
...
```{r}
#6b
count_area <- function(data, color_column) {
ggplot(data, aes(x = area, y = count, color = .data[[color_column]])) +
  geom_point(size = 3) +
  labs(x = "Area [ha]", y = "Number of Individuals") +
  scale_color_discrete(name = color_column)
}
  state_plot <- count_area(count_data, "state")
  
ggsave("state_plot.png", plot = state_plot, path = "../outputs" )
  
#6c
site_plot <- count_area(count_data, "site")
ggsave("site_plot.png", plot = site_plot, path = "../outputs")
```
